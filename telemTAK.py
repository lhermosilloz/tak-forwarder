...
class TelemTAK:
    ...
    def build_cot(self, lat, lon, alt, heading, uid, callsign):
        ...
        if self.stream_path:
            video_block = f"""<__video><ConnectionEntry uid="{uid}" alias="{callsign} Camera" address="{self.tak_host}" port="8554" path="/{self.stream_path}" protocol="rtsp" type="raw" roverPort="false" ignoreEmbeddedKLV="false" bufferTime="0" networkTimeout="10" rtspReliable="1"/></__video>"""
        else:
            video_block = ""
