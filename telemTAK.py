...
class TelemTAK:
    ...
    def build_cot(self, lat, lon, alt, heading, uid, callsign):
        ...
        if self.stream_path:
            video_block = f"""<__video><ConnectionEntry uid="{uid}-video" alias="{callsign} Camera" address="{self.tak_host}" port="8554" path="/{self.stream_path}" protocol="rtsp" type="raw" roverPort="false" ignoreEmbeddedKLV="false" bufferTime="0" networkTimeout="10" rtspReliable="1"/></__video>"""
            # video_block = f"""<__video><ConnectionEntry uid="{uid}-video" alias="{callsign} Camera" protocol="rtsp" type="raw" rover="false" ignoreEmbeddedKLV="false" buffer="0" timeout="10" rtspReliable="1" user="{self.video_user}" password="{self.video_pass}" url="rtsp://{self.video_user}:{self.video_pass}@{self.tak_host}:8554/{self.stream_path}"/></__video>"""
            # video_block = f"""<__video><ConnectionEntry uid="{uid}-video" alias="{callsign} Camera" address="{self.tak_host}" port="8554" path="/{self.stream_path}" protocol="rtsp" type="raw" rover="false" ignoreEmbeddedKLV="false" buffer="0" timeout="10" rtspReliable="1" user="{self.video_user}" password="{self.video_pass}"/></__video>"""
            # video_block = f"""<__video><ConnectionEntry uid="{uid}-video" alias="{callsign} Camera" address="rtsp://{self.video_user}:{self.video_pass}@{self.tak_host}:8554/{self.stream_path}" port="8554" path="/{self.stream_path}" protocol="rtsp" type="raw" rover="false" ignoreEmbeddedKLV="false" buffer="0" network_timeout="10" rtspReliable="1"/></__video>"""
            # After rtspReliable we removed this: user="{self.video_user}" password="{self.video_pass}" url="rtsp://{self.video_user}:{self.video_pass}@{self.tak_host}:8554/{self.stream_path}"
            # video_block = f"""<__video><ConnectionEntry uid="{uid}-video" alias="{callsign} Camera" address="rtsp://{self.video_user}:{self.video_pass}@{self.tak_host}:8554/{self.stream_path}" port="8554" path="/{self.stream_path}" protocol="rtsp" type="raw" rover="false" ignoreEmbeddedKLV="false" buffer="0" timeout="0" rtspReliable="0"/></__video>""" # All this needed was the P2P 
        else:
            video_block = ""
